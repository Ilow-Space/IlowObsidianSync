...
describe('Unsuppressed Vault Write Echo Bug', () => {
    ...
    beforeEach(async () => {
        ...
        appMock = {
            vault: {
                ...
                modify: vi.fn().mockImplementation(async (f: any, content: string) => {
                    if (f) {
                        f.content = content;
                        // Fire native Obsidian modify listeners
        ...
        // Real repository behavior: writeNote invokes app.vault.modify
        const realNoteRepoMock = {
        ...
        orchestrator = new NetworkOrchestrator(
            remoteStoreMock, cryptoMock, syncEngine, realNoteRepoMock, vfsController, eventBus, vi.fn(), 0, diskReconciler
        );
    ...
    it('FAILS ON UNPATCHED CODE: Unsuppressed note writes echo through VaultEventWatcher and overwrite remote edits', async () => {
        const docUuid = 'echo-bug-uuid';
        const initialContent = 'Base Content\n';
        const remoteContent = 'Base Content\nRemote Edit Line\n';
        ...
        const file = new TFile(); (file as any).path = 'Doc1.md'; (file as any).content = initialContent;
        ...
        remoteTextDoc.getText('markdown').insert(initialContent.length, 'Remote Edit Line\n');
        ...
        expect(pushedDocUpdates.length, 'NetworkOrchestrator echoed its own file write back to the server as a local edit!').toBe(0);
    });
});