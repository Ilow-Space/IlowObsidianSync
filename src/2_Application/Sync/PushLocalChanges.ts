...
export class PushLocalChangesUseCase {
    ...
    public async execute(path: string, localContent: string, key: CryptoKey): Promise<void> {
        // 1. Inform Yjs of local text changes
        const updateBinary = await this.crdtEngine.handleLocalChange(path, localContent);
        ...
        if (updateBinary && updateBinary.length > 0) {
            ...
            await this.remoteStore.pushUpdate(path, encryptedBlob);
        }
    }

    public async forceCompact(path: string, key: CryptoKey): Promise<void> {
        ...
        const remoteUpdates = await this.remoteStore.fetchUpdatesSince(path, 0);
        let maxId = 0;
        if (remoteUpdates.length > 0) {
            ...
            if (decryptedUpdates.length > 0) {
                await this.crdtEngine.applyUpdates(path, decryptedUpdates);
            }
        }
        ...
        await this.remoteStore.compactSnapshot(path, encryptedNewState, maxId);
    }
}
