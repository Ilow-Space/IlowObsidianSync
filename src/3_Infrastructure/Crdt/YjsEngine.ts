...
export class YjsEngine {
    ...
    public async getOrCreateDoc(path: string, initialContent?: string): Promise<Y.Doc> {
        if (this.activeDocs.has(path)) {
            return this.activeDocs.get(path)!;
        }
        ...
        const savedState = await this.localStore.loadDocumentState(path);
        if (savedState) {
            ...
            await this.localStore.saveDocumentState(path, state);
        }
    ...
    public async applyUpdates(path: string, updates: Uint8Array[]): Promise<Y.Doc> {
        const doc = await this.getOrCreateDoc(path);
        ...
        doc.transact(() => {
            for (const update of updates) {
                try {
                    ...
                    console.error(`YjsEngine error applying update for ${path}:`, err);
                }
            }
        });
     ...
     */
    public async handleLocalChange(path: string, newContent: string): Promise<Uint8Array | null> {
        const doc = await this.getOrCreateDoc(path);
        const yText = doc.getText('markdown');
